...
const ARTIFACT_NAMES: ArtifactNames = {
	"agent/instructions.md": "instruções",
	"agent/manifest.json": "o manifesto",
	"agent/README.md": "o arquivo de apresentação",
};
...
const INPUT_LABELS: ToolInputLabels = {
	write_agent_file: (input, pending) => {
		...
		return pending ? `Escrevendo ${name}` : `Gravou ${name}`;
	},
	...
	set_chat_title: (input, pending) => {
		...
		const verb = pending ? "Nomeando conversa" : "Conversa nomeada";
