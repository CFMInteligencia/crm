...
const OPEN_COLUMNS: SimpleTableColumn[] = [
	...
	{
		id: "share",
		srLabel: "Proporção do maior negócio",
		width: "w-24",
...
const TASK_COLUMNS: SimpleTableColumn[] = [
	...
	{ id: "overdue", header: "Atrasado", width: "w-24", align: "right" },
];
		...
		<div className="flex flex-col gap-6">
			...
			<div className="grid gap-6 @3xl/page-content:grid-cols-2">
				<Card className="min-w-0">
					<CardHeader>
						...
						<CardDescription>
							Os maiores negócios em aberto e o tempo em cada etapa
						</CardDescription>
						<CardAction>
							<Button asChild variant="contrast" size="sm">
								<Link href={workspaceUrl("/deals")}>Negócios em aberto</Link>
							</Button>
						</CardAction>
					</CardHeader>
					<CardPanel>
						{biggestOpen.length === 0 ? (
							<CardPanelEmpty>
								Nenhum negócio em aberto. Adicione um negócio ao funil.
							</CardPanelEmpty>
						) : (
							<SimpleTable
								variant="panel"
					...
					<CardHeader>
						...
						<CardDescription>
							{overdueTasks.length === 0
								? "As tarefas estão concluídas ou dentro do prazo"
								: `${formatCount(overdueTasks.length, "tarefa")} em atraso`}
						</CardDescription>
					</CardHeader>
					<CardPanel>
						{overdueTasks.length === 0 ? (
							<CardPanelEmpty>Nenhuma tarefa atrasada.</CardPanelEmpty>
						) : (
							<SimpleTable
								variant="panel"
								surface="page"
			...
			<Card className="min-w-0">
				<CardHeader>
					...
					<CardDescription>
						{mine
							? "Suas notas, tarefas e mudanças de etapa"
							: "Notas, tarefas e mudanças de etapa de toda a equipe"}
					</CardDescription>
					<CardAction>
						<Button asChild variant="contrast" size="sm">
				...
				{recentActivity.length === 0 ? (
					<CardTableEmpty>Nenhuma atividade registrada.</CardTableEmpty>
				) : (
