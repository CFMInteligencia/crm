		...
		<div className="flex min-h-0 flex-1 flex-col">
			...
			{thread?.status === "working" && !busy ? (
				<p className="border-t px-4 py-2 text-pretty text-muted-foreground text-xs sm:px-5">
					A pergunta anterior está em processamento. Aguarde para enviar outra.
				</p>
			) : null}
...
function Failure({ message }: { message: string }) {
	const hint = message.includes("not reachable")
		...
		: message.includes("not configured")
			? "Configure AGENT_BRIDGE_SECRET no aplicativo e no agente."
			: null;
				...
				<Attachment key={source.url} size="xs" state="done">
					...
					<AttachmentTrigger asChild>
						<a href={source.url} target="_blank" rel="noreferrer noopener">
							<span className="sr-only">Em aberto {source.title}</span>
						</a>
					</AttachmentTrigger>
				</Attachment>
			))}
