...
const DEAL_COLUMNS = [
	...
	{ id: "role", header: "Permissão", width: "w-[16%]" },
				...
				contact ? (
					<>
						{contact.isPrimaryContact ? (
							<StatusIndicator
								tone="success"
								label={`Contato principal em ${contact.company?.name ?? "esta empresa"}`}
							/>
						) : null}
				...
				contact ? (
					<>
						...
						<RecordActions
							...
							consequence={`Notas, conversas com agentes e pesquisas também serão excluídas. E-mails e reuniões permanecem associados à empresa.${contact.email ? ` A sincronização não importa ${contact.email} novamente. Adicione o contato manualmente para restaurá-lo.` : ""}`}
...
function Background({ brief }: { brief: NonNullable<Contact["brief"]> }) {
	...
	const lines = [
		...
		{ label: "Nível hierárquico", value: sections.seniority },
		...
		<Accordion type="single" collapsible>
			<AccordionItem value="previous">
				<AccordionTrigger variant="subtle">
					{roles.length === 1 ? "1 role" : `${roles.length} cargos`}
				</AccordionTrigger>
				<AccordionContent>
...
function WeKnowThem({
	...
	return (
		<DetailSheetSection title="Relacionamento">
			<DetailSheetProperties>
				{emails > 0 ? (
				...
				{colleagues.length > 0 ? (
					<DetailSheetProperty label="Outros vínculos" wide>
	...
	if (contact.deals.length === 0) {
		return (
			<DetailSheetEmpty
				...
				description={`${contactName(contact)} ainda não participa de um negócio. Crie o negócio na empresa e adicione os contatos.`}
			/>
		);
	}
