...
function companyConsequence(company: Company): string {
	...
	const gone =
		deals > 0
			? `Os negócios associados (${deals}) e todos os dados da empresa também serão excluídos.`
			: "Todos os dados associados também são excluídos.";

	const kept =
		contacts > 0
			? ` Os contatos associados (${contacts}) permanecem no CRM, sem empresa.`
			: "";
	...
	return (
		<RecordSheetFrame
			...
			stats={
				company ? (
					<DetailSheetStats>
						<DetailSheetStat label="Funil em aberto">
			...
			<>
				...
				{adding ? null : (
					<DetailSheetEmpty
						...
						description={`Adicione os contatos de ${company.name} para registrar ligações, e-mails e notas.`}
						action={
						...
						>
							<TableCell className="w-10 py-2.5 pl-5">
								<Tooltip>
									...
									<TooltipContent>
										{isPrimary ? "Contato principal" : "Definir como principal"}
									</TooltipContent>
								</Tooltip>
							</TableCell>
			...
			<>
				...
				{adding ? null : (
					<DetailSheetEmpty
						...
						description={`Nenhum negócio em andamento com ${company.name}. Crie um para incluí-lo no funil e na previsão.`}
						action={
							<Button variant="outline" size="sm" onClick={onAdd}>
								<Icon icon={Add} data-icon="inline-start" />
								Novo negócio
