...
import { formatCurrencyName, formatMoney } from "@crm/ui/lib/format";
...
const CURRENCY_OPTIONS = CURRENCIES.map((entry) => ({
	value: entry.code,
	label: `${entry.code} · ${formatCurrencyName(entry.code)}`,
}));
...
function currencyOptions(currency: string) {
	...
	return [
		{ value: currency, label: `${currency} — sem suporte` },
		...CURRENCY_OPTIONS,
	];
}
		...
		<DetailSheetProperty label={`In ${deal.reportingCurrency}`}>
			{deal.baseAmountCents === null ? (
				<span className="text-muted-foreground">
					Não {currency} taxa — fora dos totais
				</span>
			) : (
...
const CONTACT_COLUMNS = [
	...
	{ id: "role", header: "Permissão", width: "w-[20%]" },
				...
				deal ? (
					<>
						...
						<RecordActions
							...
							name={deal.name}
							consequence={`O histórico de etapas, notas e conversas com agentes também será excluído. ${deal.company.name} e os contatos associados permanecem no CRM.`}
			...
			stats={
				deal ? (
					<DetailSheetStats>
						...
						<DetailSheetStat label="Na etapa">
	...
	return (
		<DetailSheetBody>
			<DetailSheetSection title="Etapa">
				...
				{deal.closedReason ? (
					<DetailSheetProperties>
						<DetailSheetProperty label="Encerrado">
							{deal.closedAt ? (
					...
					<InlineField
						...
						onSave={(next) => {
							...
							if (!Number.isFinite(parsed) || parsed < 0) {
								toast.error("Informe um número para o valor.");
								return;
							}
			...
			<DetailSheetSection title="Descrição">
				<InlineTextArea
					...
					placeholder={`O que ${deal.company.name} está comprando, por que agora e quais são os obstáculos.`}
	...
	return (
		<DetailSheetSection title="Situação atual">
			<DetailSheetProperties>
				...
				<DetailSheetProperty label="Na etapa desde">
				...
				<DetailSheetProperty label="Participantes" wide>
					{deal.contacts.length === 0 ? (
						<span className="text-muted-foreground">
							Nenhum contato de {deal.company.name} associado.
						</span>
					) : (
		...
		return (
			<>
				...
				{adding ? null : (
					<DetailSheetEmpty
						icon={UserMultiple}
						title="Nenhum contato neste negócio"
						description={`Adicione os contatos de ${deal.company.name} que participam deste negócio.`}
						action={
				...
				{deal.contacts.map((contact) => (
					...
					>
						...
						<TableCell className="truncate px-1 py-2.5">
							<InlineTextCell
								label={`Papel de ${contactName(contact)} neste negócio`}
								value={contact.role}
