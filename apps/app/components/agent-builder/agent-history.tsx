							...
							<span className="min-w-0 flex-1">
								<span className="flex flex-wrap items-center gap-x-3 gap-y-1">
									<span className="font-semibold text-sm">
										Execução nº{String(runNumbers.get(run.id)).padStart(3, "0")}
									</span>
									<span
										className={cn(
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							O agente para e a execução fica cancelada. Notas, tarefas e
							mensagens já criadas permanecem.
						</AlertDialogDescription>
					</AlertDialogHeader>

					<AlertDialogFooter>
						<AlertDialogCancel>Manter execução</AlertDialogCancel>
						<AlertDialogAction
							variant="destructive"
							onClick={() => {
	...
	return (
		<div className="min-w-0 border-t">
			<div className="grid grid-cols-2 gap-x-4 gap-y-3 border-b bg-background px-4 py-3 sm:min-h-[58px] sm:grid-cols-4 sm:items-center sm:gap-0 sm:px-5 sm:py-2">
				<RunMeta label="Acionamento" value={humanStatus(run.triggerType)} />
				<RunMeta
					label="Iniciado por"
			...
			<div>
				...
				{run.eventsTruncated ? (
					<div className="flex min-h-9 items-center border-t px-4 py-2 text-warning text-xs sm:px-5">
						Exibindo os primeiros {events.length} de {run.totalEvents} passos.
						Esta execução é muito longa para exibição completa.
					</div>
				) : null}
			</div>
		</div>
		...
		<div className="flex min-w-0 flex-col gap-4 sm:gap-6">
			...
			<div className="min-w-0 overflow-hidden rounded-lg border bg-card">
				<div className="hidden h-9 items-center border-b bg-background px-5 text-muted-foreground text-xs sm:flex">
					<span className="w-[166px] shrink-0">Horário</span>
					<span className="min-w-0 flex-1">Alterar</span>
				...
				{visible.length === 0 ? (
					<p className="px-5 py-12 text-center text-muted-foreground text-sm">
						Nenhuma alteração corresponde ao filtro.
					</p>
				) : null}
			</div>
		</div>
