...
const BUILDER_STEPS = ["Escopo", "Instruções", "Manifesto", "Revisar"] as const;
							...
							<MessageScrollerItem messageId="shared-chat-notice">
								<div className="rounded-lg border bg-card px-4 py-3 text-sm">
									...
									<p className="mt-1 text-muted-foreground text-xs">
										Você pode ler esta conversa. Apenas o proprietário pode
										continuar ou alterá-la.
									</p>
								</div>
							</MessageScrollerItem>
	...
	return (
		<header className="flex h-12 shrink-0 items-center gap-2 border-b px-4 sm:gap-2.5 sm:pr-4 sm:pl-5">
				...
				{working ? (
					<span className="flex shrink-0 items-center gap-2 text-muted-foreground text-xs">
						<Icon
							...
							motion="none"
						/>
						<span className="sr-only">Executando em segundo plano</span>
			...
			<Button asChild variant="ghost" size="icon-sm">
				<Link href={workspaceUrl("/chat")} aria-label="Iniciar conversa">
					<Icon icon={Add} />
				</Link>
			</Button>
			...
			>
				...
				{failed ? (
					<p className="mt-2 text-destructive text-xs">
						{error ?? "Não foi possível enviar esta mensagem."}
					</p>
				) : null}
			</div>
		</div>
	...
	return (
		<div className="flex h-7 items-center">
			<Button
				variant="ghost"
				size="icon-xs"
				aria-label="Copiar resposta em Markdown"
				onClick={() => {
			...
			<div className="overflow-hidden rounded-lg border bg-card">
				...
				<footer className="flex items-center gap-2 border-t bg-muted px-4 py-3">
					...
					>
						<AsyncButtonContent
							status={stop.status}
							pendingLabel="Interrompendo"
							successLabel="Interrompendo"
...
function BuilderFailureCard({
	...
	const message =
		failure.kind === "rate-limit"
			? "O modelo atingiu o limite do Vercel AI Gateway. Aguarde ou adicione créditos na Vercel."
			: failure.kind === "restricted"
				? "Este modelo exige créditos pagos do AI Gateway. Adicione créditos na Vercel e tente novamente."
				: failure.kind === "credits"
					? "O Vercel AI Gateway está sem créditos. Adicione créditos e tente novamente."
					: "O criador de agentes não concluiu a solicitação. Tente novamente.";
		...
		<div className="flex flex-col gap-5">
			...
			<AgentCardShell name={manifest.name ?? agent.name} status="Privado">
				<div className="flex flex-col gap-2 p-4">
					...
					<ReviewRow label="Depois" value={manifest.action} />
				...
				<AgentCardFooter note="Ambiente isolado · credenciais permanecem protegidas">
	...
	const run = useMutation(
		trpc.agents.runNow.mutationOptions({
			onSuccess: async () => {
				...
				toast.success("Execução do agente adicionada à fila.");
			},
			onError: (error) => toast.error(error.message),
		}),
	);
		...
		<div className="flex flex-col gap-[18px]">
			<div className="flex flex-col gap-1">
				...
				<p className="text-muted-foreground text-sm leading-5">
					Agente Eve criado e ativado para a equipe com acesso limitado ao CRM e
					às integrações.
				</p>
			</div>
			<AgentCardShell name={agent.name} status="Ativo">
				<div className="flex flex-col gap-2 p-4">
					...
					/>
					<ReviewRow label="Executa em" value="Eve · ambiente isolado" />
					<ReviewRow
						label="Responsável"
			...
			<div>
				...
				{["Adicionar integrante à notificação"].map((suggestion) => (
					<button
						key={suggestion}
						type="button"
		...
		<main className="flex flex-1 items-center justify-center p-8">
			<div className="max-w-md text-center">
				...
				<p className="mt-2 text-muted-foreground text-sm">
					Esta conversa não existe ou você não tem acesso.
				</p>
				<Button asChild variant="outline" className="mt-5">
					<Link href={workspaceUrl("/chat")}>Iniciar conversa</Link>
				</Button>
			</div>
		</main>
	);
