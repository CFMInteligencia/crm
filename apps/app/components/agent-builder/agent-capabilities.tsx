...
const ACTION_LABELS = new Map([
	["slack.message.post", "Publicar mensagem"],
	["crm.activity.create", "Adicionar nota ou tarefa ao registro"],
	["run.summary", "Resumir a execução"],
]);
...
export function AgentCapabilities({
	...
	const join = useMutation(
		trpc.slack.joinChannel.mutationOptions({
			onSuccess: async () => {
				...
				toast.success("Convite para o Comp AI solicitado.");
			},
			onError: (error) => toast.error(error.message),
		}),
	);
	...
	if (!capabilities.readable) {
		return (
			<Alert variant="warning">
				<Icon icon={Warning} />
				<AlertTitle>Não foi possível ler o manifesto desta versão</AlertTitle>
				<AlertDescription>
					{capabilities.problem ?? "O formato do manifesto não é reconhecido."}
				</AlertDescription>
			</Alert>
		);
	}
	...
	const blocked = everyActionOff
		? "Mantenha pelo menos uma ação ativa para salvar o agente."
		: scopeEmptied
			? "Adicione um registro. Uma lista vazia permite acesso a todos os registros."
			: null;
		...
		<div className="flex flex-col gap-9">
			{current ? (
				<Section
					...
					summary="O Comp AI entra neste canal ao salvar."
					title="Local"
				>
					<ChannelPicker
			...
			<Section
				summary="Ações desativadas aqui não são executadas."
			...
			<Section
				summary={
					...
					"Dados consultados para executar a tarefa."
				}
				title="Dados acessíveis"
			>
			...
			<SaveBar
				description={
					blocked ??
					(channelChanged
						? `Comp AI entra em #${to}. Ele permanece em #${from} até ser removido.`
