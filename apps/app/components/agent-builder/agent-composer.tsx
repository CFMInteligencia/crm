			...
			<div className="flex h-7 items-center justify-between">
				...
				>
					<AsyncButtonContent
						...
						errorLabel={<span className="sr-only">Falha no envio</span>}
					>
						<Icon icon={ArrowUp} />
					</AsyncButtonContent>
				</Button>
...
function ComposerEditor({
	...
	const placeholder =
		mode === "home"
			? "Pergunte sobre o CRM ou automatize uma tarefa…"
			: "Enviar mensagem";
	const commit = () => {
	...
	return (
		<TokenField
			...
			aria-label="Enviar mensagem ao criador de agentes"
			aria-multiline="true"
		...
		>
			<PopoverTrigger asChild>
				<Button
					variant="ghost"
					size="icon-sm"
					aria-label="Mencionar registros e integrações"
					disabled={disabled}
				>
					<Icon icon={Add} />
				</Button>
			...
			<PopoverContent size="fit" align="start" side="top" className="w-80">
				...
				<div className="max-h-72 overflow-y-auto p-1">
					{connectedGoogle.map((source) => {
						...
						const label = calendar ? "Agenda Google" : "Gmail";
						return (
							<ResourceButton
								key={source.source}
...
async function readFiles(
	...
	for (const file of Array.from(files).slice(0, 5)) {
		...
		if (file.name.length > 180) {
			toast.error("O nome do arquivo é muito longo.");
			continue;
		}
		if (file.type.length > 120) {
			toast.error(`${file.name} tem um tipo não aceito.`);
			continue;
		}
