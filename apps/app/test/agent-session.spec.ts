...
describe("record context", () => {
	it("asks about the thing you are actually looking at", () => {
		expect(recordCopy("contact").title).toBe("Perguntar sobre esta pessoa");
		expect(recordCopy("company").title).toBe("Perguntar sobre esta empresa");
		expect(recordCopy("deal").title).toBe("Perguntar sobre este negócio");
	});
	...
	it("offers questions that suit the record", () => {
		...
		expect(recordCopy("contact").suggestions[0]).toBe("Quem é esta pessoa?");
	});
